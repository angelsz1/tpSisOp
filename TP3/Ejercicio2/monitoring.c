            ...
            if (event->len) {
                if (event->mask & IN_CREATE) {
                    if (event->mask & IN_ISDIR) {
                        ...
                        fprintf(file,"El directorio %s%s%s fue creado.\n", path, "/",event->name);
                        fclose(file);
                        ...
                        *pHilo = hilo;
                        ...
                        FILE * file = fopen(LOG_FILE, "a");
                        fprintf(file,"El archivo %s%s%s fue creado.\n",path, "/", event->name);
                        fclose(file);
                    }
                }
                    ...
                    if (event->mask & IN_ISDIR) {
                        FILE * file = fopen(LOG_FILE, "a");
                        fprintf(file,"El directorio %s%s%s fue borrado.\n",path, "/", event->name);
                        fclose(file);
                        char* newPath = malloc(strlen(path) + strlen(event->name) + 2);
                        strcpy(newPath, path);
                        strcat(newPath, "/");
                        ...
                        FILE * file = fopen(LOG_FILE, "a");
                        fprintf(file,"El archivo %s%s%s fue borrado.\n",path, "/", event->name);
                        fclose(file);
                    }
                }
                    ...
                    if (event->mask & IN_ISDIR) {
                        FILE * file = fopen(LOG_FILE, "a");
                        fprintf(file,"El directorio %s%s%s fue modificado.\n",path, "/", event->name);
                        fclose(file);
                    }
                    else {
                        FILE * file = fopen(LOG_FILE, "a");
